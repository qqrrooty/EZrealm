...
show_menu() {
    ...
    echo "     安装前请先更新系统软件包，缺少命令可能无法安装"
    echo "     如果启动失败请检查 /root/realm/config.toml下有无多余配置或者卸载后重新配置"
    echo "     debian系统如果命令无法启动请先更新系统/软件包"
    echo "     该脚本只在debian系统下测试，未做其他系统适配，安装命令有别，可能无法启动。如若遇到问题，请自行解决"
...
deploy_realm() {
    mkdir -p /root/realm
    cd /root/realm
    wget -O realm.tar.gz https://cf.ghproxy.cc/https://github.com/zhboner/realm/releases/download/v2.7.0/realm-x86_64-unknown-linux-gnu.tar.gz
